...
const Cart: React.FC<CartProps> = ({
  ...
  // Check authentication status when cart opens
  useEffect(() => {
    if (isOpen) {
      checkAuthStatus();
    }
  ...
  // MODIFIED: Handle proceed to checkout - skip form for logged in users
  const handleProceedToCheckout = async () => {
    ...
    // NEW: If user is logged in, skip checkout form and go straight to payment
    if (isLoggedIn) {
      setIsProcessing(true);
      try {
    ...
    // If not logged in, show checkout form as usual
...
const handleContinueWithoutBundle = async () => {
  setShowBundleUpsell(false);
  // Check if logged in after dismissing bundle upsell
  if (isLoggedIn) {
    // Directly proceed to payment for logged-in users
    setIsProcessing(true);
    try {
      await onCheckout(); // Call without form data
    } catch (error: any) {
...
{/* Show processing state or checkout button */}
                   <button
                   ...
                   >
                     {isProcessing ? (
                       ...
                       <>
                         <CreditCard size={20} />
                         {isLoggedIn ? 'Complete Purchase' : 'Proceed to Checkout'}
                       </>
                     )}
                   </button>
                   
                   {/* Show submit error if any */}
                   {errors.submit && <p className="text-red-600 text-sm mt-3 text-center">{errors.submit}</p>}
                 </div>
              )}
            </>
          ) : (
            <>
              {/* Checkout View (only for non-logged-in users) */}
              <div className="flex-1 overflow-y-auto p-6">
